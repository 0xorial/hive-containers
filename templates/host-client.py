...
injects the secret token at ~/.config/hive/hostd-token. The request goes to the
Mac daemon via the bridge (bridge:8765), bypassing the egress proxy.
"""
import json, os, pathlib, sys, urllib.request

TOKEN_FILE = pathlib.Path.home() / ".config" / "hive" / "hostd-token"
URL = os.environ.get("HIVE_HOSTD_URL", "http://bridge:8765/run")
...
# go straight to the bridge, never through the egress proxy
